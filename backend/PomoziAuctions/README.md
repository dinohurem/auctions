...
To update the database use this command from the Web project folder (replace `PomoziAuctions` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../PomoziAuctions.Infrastructure/PomoziAuctions.Infrastructure.csproj -s PomoziAuctions.Web.csproj
```
